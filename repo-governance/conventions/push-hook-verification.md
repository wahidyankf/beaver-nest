...
- Run a consumer bootstrap suite — [HIPPO's](../../.github/scripts/test-hippo-bootstrap.sh) or [RHINO's](../../.github/scripts/test-rhino-bootstrap.sh) — through one HIPPO-guarded call when a pushed range changes that consumer's pinned bootstrap, its lock, its policy example, its own suite, or its behaviour corpus. Each boundary is routed separately, so an unrelated push pays for neither. The RHINO call also lints its bootstrap and suite, because that wrapper is repository-owned and no other gate reads it. It runs on the workstation, so it cannot observe Linux-only behaviour; it exists to reject a timestamp read that skips the platform branch and a scenario corpus that no longer matches its implementations before the push rather than after it.
