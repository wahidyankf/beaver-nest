...
- Run the [`test:quick` quality gate](../development/quality-gates.md) through HIPPO-guarded, `--parallel=1` `nx affected` for every non-deleted ref being pushed, using `origin/main` as the base and the pushed local commit as the head. Only affected projects that define `test:quick` participate.
