...
Never bypass a failing or pending gate, an unresolved conversation, or branch protection. A user may waive a specific gate for a specific merge by naming it; that waiver covers nothing else, and it never covers the leak review. Repairing a gate failure follows the [push-hook verification convention](push-hook-verification.md): fix the cause.
