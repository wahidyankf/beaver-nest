...
The repository's pre-push hook follows the [push-hook verification convention](conventions/push-hook-verification.md). It runs the [`test:quick` quality gate](development/quality-gates.md) for affected projects that define the target, and also runs governance, documentation-map, and Mermaid-accessibility checks when pushed commits change Markdown anywhere or any content under `docs/`.
