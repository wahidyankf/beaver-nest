...
```sh
tools/resource-guard/resource-guard run --class ephemeral -- npm exec -- nx run -p badakmini-cli -t test:repo --skipNxCache
```
...
```sh
tools/resource-guard/resource-guard run --class ephemeral -- npm exec -- nx run -p badakmini-cli -t test:coverage:behaviour --skipNxCache
```
