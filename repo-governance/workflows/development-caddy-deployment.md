...
- Routine releases fail closed under one host lock when it is owned, the inactive slot is occupied, platform memory pressure is not normal, compressor occupancy reaches 37.5%, or the 9 GiB effective-memory, four-unit release plus two-unit safety, and 13 GiB disk envelope is unavailable. CPU admission uses runtime available parallelism. Production uses ports `4000`/`4001`, Caddy uses `4100`, E2E leases `4010`–`4019`, and development leases `4020`–`4029`.
