...
- Routine releases use one host-wide transaction and fail closed when another release owns the lock, the inactive production slot is occupied, recent swap-in is present, or the 9 GiB memory, four-core release plus two-core safety, and 13 GiB free-disk admission envelope is unavailable. Production uses ports `4000`/`4001`, Caddy uses `4100`, E2E leases `4010`–`4019`, and development leases `4020`–`4029`.
