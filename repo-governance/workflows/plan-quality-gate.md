...
10. `delivery.md` needs ordered `[AC-...]` tasks with input, action, outcome, proof, checkpoints; split mixed work. Prefer `[AI]`; use `[HUMAN]` only for unavailable authority. Include rules propagation when possible, exact canonical C4 updates for documented architecture, and Playwright tab/context cleanup proof when used.
...
12. Testing names unit/integration/behavior/E2E targets, changes, safe AI steps, setup, results, cleanup, evidence. Playwright work names failure-safe cleanup for created tabs/contexts and explicit handoffs. Authentication follows [test identities](../development/test-identities.md).
