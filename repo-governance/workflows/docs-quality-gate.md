...
Audit human-facing documents for stale, obsolete, misplaced, and unreadable documents, handing every finding to [docs propagation](docs-propagation.md) instead of editing, and audit again until two consecutive audits are clean.
...
- `max-iterations` (optional; default `7`): the ceiling on audits.

## Sequence

1. **Freeze the snapshot:** scope, Git revision, and uncommitted paths. A material external change, never propagation's own repairs, ends the run as `input-changed`, never restarting it.
...
6. **Hand over or count a clean audit.** An audit is clean when the ledger is clear and this command passes:
...
7. **Audit again** from step 1 with the same scope and a fresh snapshot. Two consecutive clean audits end the run `pass`. Continue only while open findings strictly decrease; when they stop, or after `max-iterations` audits, end `partial`, giving each remaining finding a durable owner: fixed, filed as a plan idea or backlog item, or asked through grill-me.

## Exit

Outputs: `status` (`pass`, `partial`, `input-changed`, or `fail`), the number of audits run, and the ledger, written under ignored `local-tmp/`.

A handoff is not a blocked result: the caller runs propagation and then the next audit without another request. An input change ends the run with its ledger kept; an audit or propagation that cannot run ends it `fail`. A result authorizes no commit or push.

## After a Finding

This repository records **repair to zero findings**:
...
The ceiling is `max-iterations`, declared in this gate because no shared iteration ceiling exists here. Either way the gate never edits a document.
