...
- A commit or push changes repository history only; it does not update a running backend or prove which revision it serves. Before reporting an active-service change complete, verify the routed backend serves the intended revision or observable behaviour. If it does not, complete a no-downtime candidate cutover first.
