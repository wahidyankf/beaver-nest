...
| `test:coverage:*`         | Run one documented coverage slice. Numeric thresholds may exceed 99%; exclusions must be narrow, limited to generated or non-runtime code, and documented in the project README.                                                                                         |
| `test:coverage:behaviour` | Prove every behavior step has exactly one binding and every binding is used without a slow runtime boundary. Use static completeness or numeric binding coverage. Define only for supported BDD adapters.                                                                |
...
| `test:quick`              | Compose the project's fast completion gate in deterministic order. Code projects use `typecheck` → `lint` → `test:unit` → applicable coverage slices; end-to-end harnesses use `typecheck` → `lint`. Append applicable fast behavior coverage; never include `test:e2e`. |
