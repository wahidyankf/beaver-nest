...
| `test:behaviour:compliance` | Validate executable behavior specifications and their bindings without crossing a slow runtime boundary. Define it only for projects that adopt a supported BDD adapter.                                                                                                |
...
| `test:quick`                | Compose the project's fast completion gate in deterministic order. Code projects use `typecheck` → `lint` → `test:unit` → applicable coverage slices; end-to-end harnesses use `typecheck` → `lint`. Append applicable fast compliance gates; never include `test:e2e`. |
