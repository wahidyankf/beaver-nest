...
- Unit is mandatory and has no exemption. Implement integration and E2E by default whenever their boundaries can express the scenario. When a scenario fundamentally cannot be exercised at either boundary, add the corresponding scenario-level `@integration-exempt` or `@e2e-exempt` tag; a scenario may carry one or both. Immediately precede each tag with its own `# Exemption(<layer>): <boundary reason>; alternative-proof: <Nx test target> / <scenario>`. The reason must describe a boundary mismatch, not difficulty, runtime, flakiness, cost, or unfinished work; the named alternative must prove the omitted concern. Never place exemptions on a Feature, Rule, or Background; never use `@unit-exempt` or `@no-*`; and never exempt all available proof.
