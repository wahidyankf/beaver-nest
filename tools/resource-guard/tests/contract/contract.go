...
const (
	...
	FeaturePath = "../../../../specs/tools/resource-guard/behaviours"
...
var Definitions = []StepDefinition{
	...
	{`^a host sample with 29 GiB of free disk$`},
	...
	{`^status returns schema version 2 and a resource assessment$`},
	...
	{`^a release host with eight execution units and safe memory$`},
	{`^release admission is assessed$`},
	{`^three CPU samples at or below 25 percent are required$`},
	{`^a release summary with one health failure$`},
	{`^release overlap evidence is assessed$`},
	{`^the release evidence is rejected$`},
	{`^the resource guard Nx build configuration$`},
	{`^build artifact caching is inspected$`},
	{`^compiled binaries are excluded from the Nx cache$`},
...
var ApprovedExemptions = map[string][]Exemption{
	Unit: {
		{"Nx does not retain compiled build snapshots", "requires repository project configuration"},
	...
	E2E: {
		...
		{"Storage warning blocks admission without pretending it is transient", "requires synthetic disk evidence"},
		...
		{"Release admission reserves six CPU units", "requires synthetic release host samples"},
