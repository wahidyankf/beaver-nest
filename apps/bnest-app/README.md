...
The deployment targets require machine-local `BNEST_DEPLOY_ROOT`, `BNEST_RUNTIME_ROOT`, `BNEST_DEPLOY_COOKIE_FILE`, and `BNEST_DEPLOY_SECRET_KEY_BASE_FILE` values outside this repository; release builds also require `BNEST_DEPLOY_WORKTREE`. Caddy binds only to loopback but its site address must remain host-agnostic so Tailscale Serve can forward private HTTPS requests with their original Host header. Their safe creation, promotion, and rollback sequence is defined by the [Caddy deployment workflow](../../repo-governance/workflows/development-caddy-deployment.md).
