...
This project owns the OTP application, Phoenix endpoint and router, LiveViews, Codex process bridge, server-rendered UI, browser assets, Progressive Web App (PWA) manifest and service worker, and the unit and local-only integration adapters for Bnest's canonical Gherkin behaviour. It includes the child-friendly `/apps/sifat-allah` revision activity, the SQLite-backed daily scheduler, and independently verified production SQLite backups. It also owns the authenticated family chat feature: an Absinthe GraphQL schema/resolver layer (`family_chat_rooms`/`family_chat_room`/`family_chat_messages` queries, `send_family_chat_message` mutation, `family_chat_message_committed` subscription), the `FamilyChatController` route that renders the GraphQL-driven room shell (not a LiveView), and Web Push delivery with retry and a retention job dispatched through a second `Scheduler.Registry` handler. A message may answer one earlier message in the same room: `send_family_chat_message` accepts an optional `replyToMessageId`, and every message type exposes a nullable `replyTo` carrying the answered message's id, sender, and a bounded preview of its text. The whole feature is implemented and tested behind the `BNEST_FAMILY_CHAT_ENABLED` flag, which still defaults to off; quoting is gated independently by `BNEST_FAMILY_CHAT_REPLY_ENABLED`, which also defaults to off. Browser-level acceptance belongs to [bnest-app-be-e2e](../bnest-app-be-e2e/README.md) (backend boundary, no reverse proxy) and [bnest-app-fe-e2e](../bnest-app-fe-e2e/README.md) (frontend/PWA, full Caddy rollout); the reusable Elixir BDD engine belongs to [ex-bdd](../../libs/ex-bdd/README.md).
...
## Shared behaviour and boundaries

The canonical [Bnest specifications](../../specs/apps/bnest/README.md) define Bnest's current system, container, component, data, and process boundaries, split into [`app-be`](../../specs/apps/bnest/app-be/README.md) (server boundary) and [`app-fe`](../../specs/apps/bnest/app-fe/README.md) (browser/PWA boundary); `bnest-app` aggregates both roots' unit and local-only integration adapters. The same recursively discovered features, expanded scenarios, and shared step bindings run at every application level:
...
- The integration adapter uses real in-process Phoenix components and isolated local resources. It may start a loopback endpoint on a leased port when a scenario needs a real socket, and must stop it in the same test. Browsers, non-loopback network access, and the routed public origin remain forbidden.
- The browser adapter for `app-be` lives in `bnest-app-be-e2e`; the browser adapter for `app-fe` lives in `bnest-app-fe-e2e`. Each exercises exactly the root it owns.

`test:coverage:behaviour` statically checks the unit and integration adapters this project owns against the aggregate of both roots; each E2E project's own `test:coverage:behaviour` checks its browser adapter against the one root it owns. It rejects empty features, scenarios without explicit `When` and `Then`, undefined or ambiguous steps, unused bindings, incomplete drivers, invalid exemption tags, and unit or integration boundary-policy violations. Unit runs every scenario. Integration skips only scenarios carrying a valid scenario-level `@integration-exempt`; the browser projects apply `@e2e-exempt`, and both may annotate one scenario when independently justified. Static completeness does not prove that a binding is substantive, so changes to behaviour or adapters also follow the repository's [one-by-one implementation review](../../repo-governance/workflows/gherkin-implementation-review.md).

`test:unit` is the only numeric gate and must stay at least 99%. It measures resource-free domain code: chat, learning, Codex model access/catalog/session/settings, the data-repository facade and its backend dispatch, credential and session-lifecycle logic, browser-source normalization, authorization, PWA metadata, and JSON rendering. Generated Phoenix code, test scaffolding, CLI tasks, HTTP/LiveView adapters, filesystem stores, Argon2, Codex model discovery, and the identity adapter that resolves the active store from on-disk configuration sit outside that denominator, because a unit test may not touch the resources they exist to wrap. `test:integration` still executes them; it reports a pass or fail rather than a measured percentage.

Two splits keep that denominator honest rather than widening the exclusion list. `BnestApp.Codex.ModelDiscovery` owns the Node subprocess that lists models, so `ModelCatalog` keeps only validation and state. `BnestApp.Identity.Login` owns credential verification and session revocation against any store, so `BnestApp.Identity` is left as the adapter that decides which store is active. The supervisor builds the flat store, so `BnestApp.DataRepository` never opens a path itself.
...
The authenticated home page at `/` is the child-friendly entry point and links to `/chat`, `/apps/sifat-allah`, and the authenticated `/data-migration` compatibility flow. A logged-out request to `/` redirects to `/login`, so protected home actions never render without a session. One-time `/setup` creates every initial account and permanently closes; `/login` then protects family data. There is no public registration or password-recovery flow.

## Identity and Runtime Data

Production SQLite data defaults to `~/bnest/data/prod/`; repository `data/prod/` is only a verified legacy migration source until retirement. Each test uses the same marked run identifier in a flat-file fixture root below `data/test/runs/` and a SQLite root below `~/bnest/data/test/runs/`; either root being absent, shared, or mismatched fails closed. Initial accounts have a normalized username, one or more `children`, `parents`, and `admin` roles, and an Argon2id password verifier. Passwords have no application character-count rule, but each must contain a letter, number, and punctuation mark such as `_`. Bootstrap, username, account, and session operations follow the active repository phase, so `sqlite_primary` never falls back to retired flat identity files. The browser receives only a persistent opaque session cookie; the server stores its SHA-256 digest, has no automatic session expiration, and lets one user remain logged in independently in multiple browsers. Logout revokes only that browser session. Cross-user data access is denied regardless of role.

Managed production slots always start with `BNEST_COOKIE_SECURE=true` and `BNEST_IDENTITY_CUTOVER=true`, using the replacement account backend and login boundary instead of the legacy browser identity. Both variables accept only `true` or `false`. Local HTTP development keeps the secure-cookie override unset and may select transition mode explicitly, while production Mix configuration enables secure cookies automatically.
...
The theme picker belongs to the current page's controls rather than floating over chat content. In chat it shares the wrapped control row with the model status, repository-access badge, optional admin write control, and **Clear chat**, so narrow screens retain every control without covering the brand.
...
Every chat runner starts with read-only sandbox access, approval policy `never`, and disabled network and web search. An account with `admin` and without `children` may explicitly enable `workspace-write` for its current connected chat; parent-only accounts, invalid role sets, and any account containing `children` remain read-only. The server derives and allowlists the mode from the authenticated role set, never from a browser-supplied sandbox value. The write toggle is not persisted and resets on reload, reconnect, logout, and **Clear chat**. Assistant message updates stream into the transcript as plain text. Public SDK reasoning summaries and generic activity status display in a collapsible **Thinking** section, keyed by their runner item IDs, and stay beside the final answer when a later assistant item arrives. Raw private reasoning and tool input are never stored or displayed. **Shift+Enter** submits the composer, while plain Enter remains available for multiline input.

The family chat room is browser-driven rather than a LiveView: the controller renders only the room shell, and
`assets/js/family_chat/` decides what to load and where to put the visitor. A room opens at the reading position
that device last reached — the messages committed since it, one bounded page of earlier context above them, and a
marker between the two — rather than at the oldest loaded message. A member with nothing unread, or with no stored
position, opens at the newest message. That position lives only in the visitor's own browser storage, so losing it
degrades to opening at the newest message and never affects another device. The composer keeps keyboard focus
across a send so the on-screen keyboard stays up, `Enter` sends while **Shift+Enter** continues the same message,
and sending returns the sender to their own message however far back they were reading.

Answering one message in particular is the room's only structural relationship between messages, and it is one
level deep: a reply to a reply quotes what it answers, never the chain behind it. A member opens a per-message
action menu, chooses **Reply**, and the chosen message appears above the composer until they send it or abandon
it; the target never survives a reload, because a quote the member cannot see is a quote they did not choose.
The sent reply renders a quote card inside its own bubble, on every path a message can arrive by, and activating
that card returns the reader to the original — loading earlier pages if it is above the window, and saying so out
loud rather than doing nothing when it is further back than the jump will go. The history keeps exactly one tab
stop wherever the reader is, so neither the menu trigger nor the quote card turns the conversation into a tab
sequence.
...
The test environment substitutes a deterministic model catalog and session, and browser tests start deterministic catalog and chat runners. These fixtures verify the complete model catalog, model-specific effort choices, effective model, effort, and sandbox arguments, streaming, and same-thread resume protocol, but the canonical Gherkin never requires particular model wording. A real-model smoke check should assert only discovery, protocol health, stable thread identity, and non-empty completed responses; model-quality requirements belong in representative evals with outcome-based graders.
...
- `lib/bnest_app/backup.ex` and `lib/bnest_app/backup/` own the private destination pointer, marker, independent snapshot proof, fenced publication, capacity/concurrency, receipts, and seven-WIB-day retention.
- `lib/bnest_app/family_chat.ex` and `lib/bnest_app/family_chat/` own room/message persistence, the atomic delivery commit consumed by the GraphQL subscription, and the single 160-grapheme rule every quote preview is cut by.
- `priv/sqlite_repo/migrations/20260922000000_add_family_chat_message_reply.exs` adds the nullable `reply_to_message_id` column and its partial index, and refuses to reverse once a reply exists.
- `assets/js/family_chat/message_actions.js`, `reply_target.js`, and `jump_to_message.js` own the per-message action menu, the chosen reply target shared by the composer and the outbox, and the bounded jump back to a quoted message.
- `lib/bnest_app/push_notifications.ex` and `lib/bnest_app/push_notifications/` own VAPID delivery, dispatch policy, retry, and the retention/purge job dispatched through the Scheduler registry.
...
- The pinned HIPPO owns bounded development and E2E listener leases before either runtime starts.
...
- `specs/apps/bnest/app-be/architecture.md` and `specs/apps/bnest/app-fe/architecture.md` contain the canonical as-built C4 models.
