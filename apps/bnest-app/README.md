...
The deployment targets require machine-local `BNEST_DEPLOY_ROOT`, `BNEST_RUNTIME_ROOT`, `BNEST_DEPLOY_COOKIE_FILE`, `BNEST_DEPLOY_SECRET_KEY_BASE_FILE`, and a bare-HTTPS `BNEST_PRODUCTION_ORIGIN` outside this repository; release builds also require `BNEST_DEPLOY_WORKTREE`. Candidate preparation derives `PHX_HOST` from that validated origin so routed WebSocket origin checks remain enabled. The managed release target accepts only a clean `origin/main` revision, serializes requests with a host-wide lock, checks capacity and exact port ownership, runs a fixed uncached gate manifest, builds once in a detached worktree, records a digest and empty migration manifest, proves the inactive slot and routed revision, qualifies ten distinct synthetic identities in three groups at an isolated exact origin, exercises 10 anonymous routed LiveViews without reload, drains for five minutes, and retains only the active and previous artifacts. It keeps machine-local aggregate samples and a summary for effective available memory, macOS pressure state, compressor occupancy, CPU, service RSS, disk, swap, and Caddy latency without recording origins, paths, arguments, or user data. Memory admission requires normal platform pressure, at least 9 GiB effective availability, and compressor occupancy below 37.5% of physical memory; that ceiling preserves measured margin below the host's compressor-exhaustion failure floor. CPU admission uses Node's available parallelism and reserves four execution units for the bounded release plus two for service safety; on a host with 12 available units, its aggregate `ps` ceiling is therefore 600%. The final overlap proof retains the pressure and compressor guards, treats swap-in as additional pressure only below 4 GiB minimum available memory, and independently preserves a 2 GiB hard floor plus two p95 CPU cores. A non-empty migration set fails closed until an approved expand–migrate–verify adapter exists.
...
- `tools/resource-monitor.mjs` writes machine-local aggregate release evidence and a bounded peak/p95 summary.
- `tools/production-origin.mjs` validates the routed HTTPS origin and exposes only its normalized origin and hostname.
- `tools/port-lease.mjs` owns bounded development and E2E listener leases; `tools/serve.mjs` starts development on that contract.
