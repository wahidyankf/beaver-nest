...
`test:quick` runs type checking, linting, unit execution, unit coverage, and static behavior completeness. It intentionally excludes integration runtime and E2E execution. Both numeric coverage slices use Mix line coverage and fail below 99%; integration runtime and coverage share stale isolated-data cleanup and run one ExUnit case at a time because migration modules and SQLite repository lifecycle are process-global within the test VM.
