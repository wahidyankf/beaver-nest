...
The deployment targets require machine-local `BNEST_DEPLOY_ROOT`, `BNEST_RUNTIME_ROOT`, `BNEST_DEPLOY_COOKIE_FILE`, and `BNEST_DEPLOY_SECRET_KEY_BASE_FILE` values outside this repository; release builds also require `BNEST_DEPLOY_WORKTREE`. The launcher preserves its executable search path and points Codex model discovery at the permanent main checkout, so the temporary build worktree can be deleted immediately after the immutable release is built. Caddy binds only to loopback, accepts the Host header forwarded by Tailscale Serve, and forwards the HTTPS scheme to Phoenix so its secure cookies and redirects remain correct. Their safe creation, promotion, and rollback sequence is defined by the [Caddy deployment workflow](../../repo-governance/workflows/development-caddy-deployment.md).
