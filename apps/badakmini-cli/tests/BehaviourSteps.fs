...
[<When>]
let ``I inspect directory maps under "(.*)"`` (directory: string) (context: ScenarioContext) =
    try
        let inspection = Governance.inspectDirectoryMapsAt context.Root directory
        context.DirectoryCount <- Some inspection.DirectoryCount
        context.Violations <- inspection.Violations
...
[<Then>]
let ``(\d+) Markdown files were inspected`` (expected: int) (context: ScenarioContext) =
    Assert.Equal(expected, context.MarkdownFiles.Length)
...
[<Then>]
let ``stderr is empty`` (context: ScenarioContext) = Assert.Empty context.StandardError
