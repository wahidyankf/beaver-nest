...
The unit driver uses an in-memory filesystem and injected output only. The integration driver uses real temporary local files. Its policy test rejects network APIs and endpoints, including loopback, because Badakmini is a network-free governance system by design; that check encodes a product invariant and is not an instance of the integration-layer boundary, which permits an owned loopback socket. [`badakmini-cli-e2e`](../badakmini-cli-e2e/README.md) invokes the built CLI process and observes public output. `test:coverage:behaviour` statically rejects missing disk or embedded features, empty or malformed scenarios, undefined or ambiguous steps, unused bindings, incomplete drivers, and invalid exemption tags across the unit and integration adapters this project owns, without running slow scenarios; `badakmini-cli-e2e` runs the same check for the process adapter. Unit runs every scenario; integration and E2E omit only their own documented scenario-level exemption. Adapter changes also require the [manual semantic review](../../repo-governance/workflows/gherkin-implementation-review.md).
