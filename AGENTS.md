...
- **Project rule** (except `libs/ex-bdd`): maintain [specifications](repo-governance/development/specification-maintenance.md); Gherkin → failing bindings → Nx red → code → smoke. Reject placeholders/no-ops/outcome tables; unit is mandatory. Document `@integration-exempt`/`@e2e-exempt`; run the [manual review](repo-governance/workflows/gherkin-implementation-review.md).
