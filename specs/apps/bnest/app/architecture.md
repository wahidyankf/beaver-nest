...
- Routine releases are clean-revision, host-locked transactions with fixed uncached gates, capacity and port admission, immutable artifact and migration manifests, revision proofs, rollback, bounded drain, and two-artifact retention. Pre-mutation admission requires normal platform memory pressure, at least 9 GiB effective availability, and compressor use below 37.5% of physical memory. CPU admission uses the runtime's available parallelism and reserves four execution units for release work plus two for service safety. Final overlap proof retains the pressure and compressor guards, treats swap-in as additional pressure only when minimum available memory also fell below 4 GiB, and independently requires at least 2 GiB available and two CPU cores of p95 headroom.
