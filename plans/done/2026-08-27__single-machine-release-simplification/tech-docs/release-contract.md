...
2. **Pre-artifact gates:** run a fixed target manifest covering applicable quick, integration, behaviour, repository, and isolated browser tests. Any failure or indeterminate result stops before build.
3. **Build:** create one detached clean worktree, install JavaScript and production Elixir dependencies from their committed lockfiles with lock drift forbidden, create one immutable revision-addressed artifact, and manifest its revision, toolchain, content digest, passed gate evidence, and declared data/schema compatibility.
...
7. **Routed proof:** verify intended revision, HTTPS, WebSocket reconnection, current route, no duplicate action, no progress loss, and representative isolated data behaviour at the exact user origin.
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| 1     | `npm exec -- nx run -p bnest-app -t test:quick --skip-nx-cache`                                                                                                | Typecheck, lint, unit, unit coverage, and behaviour coverage pass.                                  |
...
| 4     | `npm exec -- nx run -p bnest-app-e2e -t test:e2e --skip-nx-cache -- --workers 1 --grep "An automatic LiveView reconnect"`                                      | Desktop, tablet, and mobile completed- and in-progress-turn reconnect journeys pass without reload. |
...
- `[E] specs/apps/bnest/app/behaviours/chat.feature`: require automatic transport reconnect, current-route preservation, unsent composer recovery, and persisted in-progress-turn restoration without reload.
...
- Release orchestration behaviour is proven by deterministic Node unit tests because it operates OS processes and machine-local deployment state; shared application adapters must never execute it against production.
...
- `[E] apps/bnest-app/project.json`, `apps/bnest-app/mix.exs`, `apps/bnest-app/lib/bnest_app_web/live/chat_live.ex`, `apps/bnest-app/tools/release.mjs`, `apps/bnest-app/tools/release.test.mjs`, `apps/bnest-app/test/behaviour/driver.ex`, `apps/bnest-app/test/behaviour/steps/home_page_steps.exs`, `apps/bnest-app/test/unit/support/home_page_driver.ex`, and `apps/bnest-app/test/integration/support/home_page_driver.ex`: Nx targets, complete- and in-progress-turn release gating, safe model normalization, form auto-recovery, and recovery bindings/adapters.
...
- `[E] specs/apps/badakmini/cli/behaviours/mermaid-cli.feature`, `specs/apps/badakmini/cli/behaviours/mermaid-governance.feature`, and `specs/apps/badakmini/cli/architecture.md`: exact legibility behaviour and C4 responsibility.
