...
- [Migration design](migration-design.md) owns current flat-file and future database/schema ordering, option-specific behaviour, verification, and recovery.
...
The recommendation was reassessed after the 440-second managed release and again after the repaired 450-second managed release; it remains unchanged for one or more releases every day. The original measurement retained 12.80 GiB minimum available memory, 95.95 GiB minimum free disk, two p95 CPU cores of safety headroom, zero Caddy health failures, and only one Phoenix listener after cleanup. The repaired release closed the persisted-pending-turn recovery proof gap through safe model normalization and a fixed isolated authenticated recovery gate before promotion. High cadence increases the value of one automated transaction and makes manual Alternative A, interruptive E, and version-pair-specific F less suitable. Alternative C avoids candidate startup but always consumes the second VM; B consumes it only for prepare, proof, and bounded drain. D or G can replace B when their full measured control-plane cost and continuity proof are better, not merely because they remove Caddy.
...
Consider removing Caddy only when measurements show its steady-state cost is material and a single-slot restart proves a bounded interruption, automatic recovery for every affected page, no progress loss, and deterministic cold rollback. This has the fewest processes but removes documented Caddy cutover/drain behaviour. It is not the first recommendation while page-state recovery remains unproven.
...
- Containers or an orchestrator reproduce existing single-host slot behaviour with additional infrastructure.
