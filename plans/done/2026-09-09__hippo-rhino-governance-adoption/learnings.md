...
Two things follow. First, a worktree is not a checkout until its dependencies are installed. This repository had already written that rule, in the convention this plan spent eleven phases porting to two others: "a worktree whose hooks were never activated pushes unverified work," in [the integration path](../../../repo-governance/conventions/integration-path.md), naming the exact command. It was skipped here, in the repository that wrote it, on this plan's own final change. A rule you can quote and still not follow is a rule that needs a gate behind it. Second, `--no-verify` is not the only way to skip a hook — an uninstalled hooks path does it by default, and unlike `--no-verify` it leaves no trace in the command that ran. A rule that prohibits the flag does not cover the case; only the server-side gate does.
