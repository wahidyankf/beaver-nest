...
3. **Build:** create one detached clean worktree, fetch its production dependencies from the committed lockfile with lock changes forbidden, create one immutable revision-addressed artifact, and manifest its revision, toolchain, content digest, passed gate evidence, and declared data/schema compatibility.
