...
PR #55 (Phase 8.5) fixed the *send* path so new messages carry the real session-derived `displayUsername`, but a
...
leaves the immutable column completely untouched and instead resolves the *displayed* name against the sender's
