...
rhino/
  ...
  hippo, hippo.lock     pinned guard for local heavy cargo work; HIPPO pins RHINO back
...
rhino harness parity validate              [--root <path>] [--harness <name>]...
...
| Command                             | What it checks                                                                                                                                                                                                                                                                                                                                                                  | Reads                                                                                                                          | Exit `1` when                                                                                     |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| `governance word-budget validate`   | Every Markdown file matched by a declared surface stays within that surface's word budget, counted after front matter, code fences, and link URLs are removed.                                                                                                                                                                                                                  | The declared surface globs and their fail thresholds.                                                                          | Any governed file exceeds its budget.                                                             |
| `governance directory-map validate` | Each declared tree carries a directory map that lists exactly the entries actually present — no missing row, no row for something that no longer exists, no unmapped subdirectory.                                                                                                                                                                                              | The declared mapped trees.                                                                                                     | A map and its directory disagree.                                                                 |
| `harness parity validate`           | One canonical instruction body, skill bundle, and agent roster reconcile against every declared harness: each canonical agent has exactly one adapter per harness, each skill has its wrapper where the harness declares one, no competing always-on instruction source exists, and the required credential-free capability is declared identically in each harness's own file. | The canonical roots, the full harness roster, the permitted capability and constraint vocabulary, and the required capability. | Any harness diverges from canon, or an unexpected adapter or instruction source exists.           |
| `md internal-link validate`         | Every local document link in every Markdown file resolves to a file or anchor that exists. External URLs are never fetched, resolved, or reported on.                                                                                                                                                                                                                           | The declared link-source exclusions.                                                                                           | A local target is missing.                                                                        |
| `md mermaid validate`               | Mermaid diagrams stay legible and accessible: node and edge label segments within their grapheme limits, and the colour rules the tool owns. Diagram kinds it cannot parse are reported as skipped, never silently passed.                                                                                                                                                      | The two grapheme limits.                                                                                                       | A label exceeds its limit or a colour rule is broken.                                             |
| `md word-count inspect`             | Nothing. It reports one file's governed word count — the same number the budget validator uses — so a maintainer can see the count before a gate rejects it.                                                                                                                                                                                                                    | The counting rules only; no policy.                                                                                            | Never. It is an inspector: a missing file is exit `2`.                                            |
| `repo-config validate`              | The configuration itself: known schema identity, every required field present, no unknown key silently ignored, no credential or environment value where one may never appear.                                                                                                                                                                                                  | Itself.                                                                                                                        | Never. A malformed policy is exit `2` here exactly as it is from every other command — see below. |
| `version`                           | Nothing. It prints the release identity, with `--json` emitting the exact fixed object the consumer bootstrap compares against the lock.                                                                                                                                                                                                                                        | Nothing; the identity is embedded at build time.                                                                               | Never.                                                                                            |

Only the five governance validators can report exit `1`. `md word-count inspect` and `version` answer questions rather than enforce rules, and `repo-config validate` reports configuration defects the way every other command does — as exit `2`, not as findings. That last one is deliberate and is the subtle one: it would be reasonable to call a malformed policy a _finding_ of the command whose subject is the policy, but then the meaning of exit `2` would depend on which command produced it. Keeping it uniform lets a gate treat `2` as "fix your setup" and `1` as "fix your content" without knowing which leaf ran. [The configuration contract](02-configuration-contract.md) lists the full set of conditions.

The split also keeps the numeric gate debuggable: when a word budget fails, the same count is obtainable for one file without re-running the gate or reading the tool's source.

Every validator takes `--root` and defaults to the current directory, reads its policy from the configuration at that root, and reports findings the same way; only `directory-map`, `mermaid`, and `harness parity` take a narrowing flag, and in all three cases narrowing selects what is examined and never what counts as a finding.

This grammar unifies three existing dialects; [the sibling-consumer design](04-sibling-consumers.md) sets out the mapping, including grind-in-public's `harness instruction-size validate` and its siblings. Two deliberate alignments with `ose-public`, and two deliberate divergences:
...
The `harness` namespace is top-level rather than a child of `governance` because harness parity is not a documentation-hygiene rule: it reconciles one canonical instruction body, skill bundle, and agent roster against every coding harness the repository supports. It is also where two of the three existing dialects already put it — `ose-public` ships `harness audit`, `harness bindings validate`, `harness catalog validate`, and `harness ownership validate`, and grind-in-public groups its parity check under `harness` too. Only Badakmini nests it under `governance`, so the placement follows the majority rather than inventing a fourth arrangement. All three harnesses BeaverNest declares today — Claude, Codex, and OpenCode — are equal citizens of that command and of the configuration behind it; none is the tool's default and none is special-cased in code. `--harness` may be repeated to narrow a run to named harnesses while a divergence is being fixed, and is purely a filter: omitting it checks the whole declared roster, and no filter can turn a finding into a pass.
