...
| `repo-config validate`              | The configuration itself: known schema identity, every required field present, no unknown key silently ignored, no credential or environment value where one may never appear.                                                                                                                                                                                                  | Itself.                                                                                                                        | The declared policy is malformed or incomplete.                                         |
...
Two of the eight are deliberately not validators. `md word-count inspect` and `version` answer questions rather than enforce rules, which is why neither can ever exit `1` — a caller that treats a non-zero status as "violation found" stays correct across the whole surface. The split also keeps the numeric gate debuggable: when a word budget fails, the same count is obtainable for one file without re-running the gate or reading the tool's source.
